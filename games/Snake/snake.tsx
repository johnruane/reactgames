...
const Snake = ({
  ...
  const initialFoodBoard = _cloneDeep(emptyBoard);
  initialFoodBoard[5][5] = FOOD_VALUE;

  const [displayBoard, setDisplayBoard] = useState(_cloneDeep(emptyBoard));
  ...
  const moveSnake = () => {
    ...
    if (canMove) {
       ...
       */
      const newSnake = _cloneDeep(snakeBody);
